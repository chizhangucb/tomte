...
- **Dispatch and heartbeat**: the sweep runs on the caller's `issues: [closed, labeled, unassigned, unlabeled]`, `workflow_dispatch` and the `factory-sweep` `repository_dispatch`, driven by the heartbeat; namespaces, reconciler, the interval derivation and what-is-due live in [factory/dispatcher.md](factory/dispatcher.md).
...
- **Heartbeat interval.** Run `GH_TOKEN=<token> node --experimental-strip-types factory/heartbeat/send.ts` every 30 minutes from an outside host, the only thing that sweeps on an interval (`HEARTBEAT_INTERVAL_MINUTES` in `factory/heartbeat/interval.ts` is the one place the number lives); the derivation is in [factory/dispatcher.md](factory/dispatcher.md). On an always-on machine that is `scripts/heartbeat-loop.sh`, kept alive by launchd or systemd, which pulls `main` and sleeps the interval itself so the host carries no copy of it (#326).
