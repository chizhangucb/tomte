...
- **`errorMessage` and `GH_MAX_BUFFER` added to `common.ts`** (`b0b79ed`, `c1addc1`). The buffer fixed a real ENOBUFS in the dispatcher but belonged in the gh wrapper alone. **Undone by #47**: `errorMessage` lives in `factory/lib/errors.ts` with its callers, and the buffer in `factory/lib/gh.ts`, which `common.ts` re-exported until #313 dissolved it and left every caller importing `lib/gh.ts` directly.
