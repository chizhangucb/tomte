...
**Host**:
Whatever runs the sender on the interval, which the factory does not own and cannot see: an always-on machine of a maintainer's, or a cloud scheduler. README's "what any host needs" is the whole contract a host meets -- the command, Node and `gh`, a checkout of `main`, the heartbeat's own token, the interval and the **dead-man's switch**'s ping URL -- so the choice of provider is an adopter's and no part of the factory names one. A host carries the one copy of the interval this repo cannot hold, which is why a **cloud recipe** that states it is held to the constant by a test. GitHub's `schedule` trigger is not a host: it was measured firing a small fraction of the times it should and left the caller for it (#270).
