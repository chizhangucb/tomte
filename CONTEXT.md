...
**Heartbeat**:
The `factory-sweep` dispatch sent to a target on an interval from outside GitHub. It is what drives the dispatcher's sweep and the reconciler, so it is required: GitHub's own `schedule` cron was removed from the caller in #270 because it did not reliably fire, and the daily recheck (#267) retries a repair that changed nothing. One sender covers any number of targets, and it sends to a running target something is waiting on, reading each target before waking it: a **Pause** is the first question and skips a paused target whatever is open on it (#256), then its open work, which skips an idle one (#212), then what is due on that work, which skips a target whose open subjects are all between deadlines (#264). The three skips are reported apart, because a pause that reads like an idle target is how a forgotten pause goes unnoticed, and a working target that reads like an idle one is how the interval becomes the only lever on the bill. The sender itself is one stateless command that writes no file on the host and carries nothing from one pass to the next, so every answer in a pass comes from the targets that pass read (#328). On an always-on machine what keeps it running on the documented interval is the **Loop runner**, which leaves that machine no second copy of the number to drift from; on a cloud scheduler, which carries its own copy, a host running it too slowly is caught by the ping below rather than by anything the sender remembers. After the pass it reports the pass's exit status to a **dead-man's switch**, a healthchecks.io check whose ping URL the host configures and which the sender sends nothing to when it is unset or when the pass is a dry run, so a failed pass alerts at once and a pass that never happens alerts after the check's period and grace (#325); a switch that is unreachable or slow is a line on stderr and costs the pass nothing, because watching the heartbeat may not be what stops it.
_Avoid_: cron (GitHub's word for the `schedule` trigger), the sweep (what the heartbeat triggers, not the heartbeat itself).

**Host**:
Whatever runs the sender on the interval, which the factory does not own and cannot see: an always-on machine of a maintainer's, keeping a **loop runner** alive, or a cloud scheduler running the **cloud recipe**. README's "what any host needs" is the whole contract a host meets -- the command, Node and `gh`, a checkout of `main`, the heartbeat's own token, the interval and the **dead-man's switch**'s ping URL -- so the choice of provider is an adopter's and no part of the factory names one. A host that schedules carries the one copy of the interval this repo cannot hold, which is why the **cloud recipe**'s schedule is held to the constant by a test; a host that keeps a **loop runner** alive carries none, since the loop reads the interval back out of the repo on every pass. GitHub's `schedule` trigger is not a host: it was measured firing a small fraction of the times it should and left the caller for it (#270).
_Avoid_: runner (the workflow's word for what GitHub bills), server, the launchd job (one host's own wiring, not the role).

**Cloud recipe**:
The deployable copy of a host this repo carries, for an adopter with no always-on machine: `deploy/render/Dockerfile`, an image of Node, `gh` and the repo whose command is the sender, and `render.yaml`, a Render blueprint declaring one cron job of it, auto-deploying `main` and taking its token and ping URL from an environment group a maintainer makes first (#327). Render is the one provider worked through, at its per-service monthly minimum; any other provider runs the same image or the same command, and the recipe says so rather than reading as the way the heartbeat is run.
_Avoid_: the deployment, the Render app (it is a cron job, not a service that stays up).
